...
export default function ToolPanels(props){
  ...
  useEffect(() => {
    devRef.current.addEventListener('mousemove', resizePanel);
    devRef.current.addEventListener('mouseup', stopResize);
    devRef.current.addEventListener('mouseleave', stopResize);
    return () => {
      devRef.current.removeEventListener('mousemove', resizePanel);
      devRef.current.removeEventListener('mouseup', stopResize);
      devRef.current.removeEventListener('mouseleave', stopResize);
  ...
  return(
    ...
    <div ref={devRef} className="panel-container" onMouseUp={() => stopResize()}>
            ...
            {[].concat(...rest.map((child, i) => {
                return [
                    ...
                    <div key={"panel_" + i} className="panel"
                        style={{ width: i === 0 ? `calc(100% - ${panels[0]}px - ${panels[2]}px)` : panels[i + 1], minHeight: i === 1 ? '300px' : '', overflow: i === 1 ? 'scroll' : '', overflowX: i === 0 ? 'hidden' : '' }}>
                        {child}
                    </div>
                ]
            }))}
